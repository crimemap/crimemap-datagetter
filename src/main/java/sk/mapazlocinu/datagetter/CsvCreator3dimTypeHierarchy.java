...
public class CsvCreator3dimTypeHierarchy {
	...
	public void createCsv(List<Worksheet> worksheets, Config config) throws IOException {
		...
		PrintWriter pw = new PrintWriter(new OutputStreamWriter(
				new FileOutputStream(config.getCsvFileName()+"_3dimTypeHierarchy.csv"),
		...
		Map<String,Integer> typeGroups = new HashMap<String, Integer>();
		...
		for(Worksheet ws:worksheets){
			...
			for (int y = 0; y < rows.size(); y++) {
				...
				csvrow.add(getHeaderIndex(ws.getDatatable().getRowHeaders().get(y).split("-")[1],typeGroups));
				if(ws.getDatatable().getRowHeaders().get(y).split("-")[0].equals("doplnok")){
					csvrow.add(getHeaderIndex(ws.getDatatable().getRowHeaders().get(y).toLowerCase(),types));
				}else{
					csvrow.add(getHeaderIndex(ws.getDatatable().getRowHeaders().get(y).split("-")[0],types));	
				}
...
//		headers.add(worksheets.get(0).getDatatable().getRowHeaders());
		...
		headers.add(getList(types));
		headers.add(getList(typeGroups));
